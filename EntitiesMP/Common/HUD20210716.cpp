...
#define BOMB_FIRE_TIME 1.5f
  ...
  if(/* !bSinglePlay*/true)
  {
	...
	CTString strSecrets, strKills, strSessionTime, strDeaths;
	...
	if( hud_iShowPlayers==1 || hud_iShowPlayers==-1 /*&& !bSinglePlay*/ && _penPlayer->m_bShowingTabInfo) {
		...
		DIO_DrawText(ESP_Middle, -150, ESP_Middle,  405, TRANS("Time"),  2, ESP_Middle,  C_WHITE); //elapsed time
		DIO_DrawText(ESP_Middle, -150, ESP_Middle,  440, strSessionTime, 2, ESP_Middle,  C_WHITE);
	...
	CTString strFragsLeft, strScoreLeft;
	strFragsLeft.PrintF( "%i", iFragsLeft);
	strScoreLeft.PrintF( "%i", iScoreLeft);
