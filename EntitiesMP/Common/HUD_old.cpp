...
)
{
  ...
  //_pDP->AddTexture(x_pos - half_width*2, y_pos - half_height*2, half_width*2, half_height*2, color);

  _pDP->PutTexture(&toIcon, PIXaabbox2D( PIX2D(x_pos, y_pos), PIX2D(x_pos+half_width*2, y_pos+half_height*2)), color);
  //_pDP->FlushRenderingQueue();
}
...
//      HUD_DrawIcon(   fCol,      fRow, _toHealth, _colHUD, fNormValue, FALSE);
  ...
  if(/* !bSinglePlay*/true)
  {
	...
	//Render table score
	if ( _penPlayer->m_bShowingTabInfo) {
		DIO_DrawBcg( ESP_Middle,    0, ESP_Middle,    0, ESP_Middle, 1280, ESP_Middle, 768, 0x00000080); // table background
		DIO_DrawBcg( ESP_Middle,    0, ESP_Middle, -290, ESP_Middle, 1240, ESP_Middle,   4, 0xFFFFFF80); // line
		DIO_DrawText(ESP_Middle, -550, ESP_Middle, -250, TRANS("Ping"), 2, ESP_Middle, _colHUD); // captions
		DIO_DrawText(ESP_Middle, -250, ESP_Middle, -250, TRANS("PLAYER"),  2, ESP_Middle, _colHUD);
		DIO_DrawText(ESP_Middle,   50, ESP_Middle, -250, TRANS("Health"),  2, ESP_Middle, _colHUD);
		DIO_DrawText(ESP_Middle,  150, ESP_Middle, -250, TRANS("Armor"),   2, ESP_Middle, _colHUD);
		DIO_DrawText(ESP_Middle,  300, ESP_Middle, -250, TRANS("Money"),   2, ESP_Middle, _colHUD);
		//DIO_DrawText(ESP_Middle,  450, ESP_Middle, -250, TRANS("Weapon"),  2, ESP_Middle, _colHUD);
		DIO_DrawText(ESP_Middle,  500, ESP_Middle, -250, TRANS("Ammo"),    2, ESP_Middle, _colHUD);
	}
    ...
    { // get player name and mana
	  ...
	  const INDEX      iAmmo    = penPlayer->m_penWeapons !=NULL? penPlayer->GetPlayerWeapons()->GetAmmo():0;
	        INDEX      iPing    = ceil(penPlayer->en_tmPing*1000.0f);
			INDEX      iListValuePosY    = -210+(40*i);
      CTString strScore, strMana, strFrags, strDeaths, strHealth, strArmor, strMoney, strAmmo, strLatency;
      ...
      strMoney.PrintF(  "$%i", iMoney/*/100*/);
	  strLatency.PrintF( "%i ms", iPing);
      ...
      // hitrowski was here
      if (penPlayer->GetPlayerName().GetHash() == ((CPlayer*)&*_penPlayer)->GetPlayerName().GetHash()) {
      ...
      if( hud_iShowPlayers==1 || hud_iShowPlayers==-1 /*&& !bSinglePlay*/ && _penPlayer->m_bShowingTabInfo) {
		...
		if (penPlayer == _penPlayer) {
			DIO_DrawBcg( ESP_Middle,    0, ESP_Middle, -230+(40*i), ESP_Middle, 1240, ESP_Middle,   40, 0xFFFFFF44); // line for current player
		} 
		DIO_DrawText(ESP_Middle, -550, ESP_Middle, iListValuePosY, strLatency, 2, ESP_Middle, _colHUD  |_ulAlphaHUD); // values
		DIO_DrawText(ESP_Middle, -250, ESP_Middle, iListValuePosY, strName,    2, ESP_Middle, colScore |_ulAlphaHUD);
		DIO_DrawText(ESP_Middle,   50, ESP_Middle, iListValuePosY, strHealth,  2, ESP_Middle, colHealth|_ulAlphaHUD);
		DIO_DrawText(ESP_Middle,  100, ESP_Middle, iListValuePosY, "/",        2, ESP_Middle, colScore |_ulAlphaHUD);
		DIO_DrawText(ESP_Middle,  150, ESP_Middle, iListValuePosY, strArmor,   2, ESP_Middle, colArmor |_ulAlphaHUD);
		DIO_DrawText(ESP_Middle,  300, ESP_Middle, iListValuePosY, strMoney,   2, ESP_Middle, _colHUD  |_ulAlphaHUD);
		/*DIO_DrawText(ESP_Middle,  450, ESP_Middle, iListValuePosY, "Weapon",   2, ESP_Middle, colScore |_ulAlphaHUD);
		DIO_DrawText(ESP_Middle,  550, ESP_Middle, iListValuePosY, strAmmo,    2, ESP_Middle, colScore |_ulAlphaHUD);*/
		
		if (penPlayer->m_penWeapons!=NULL) { 
			
			 if (!GetSP()->sp_bInfiniteAmmo) {
				for( INDEX ii=7, index=0; ii>=0; ii--, index++) {
				  i = aiAmmoRemap[ii];
				  // if no ammo and hasn't got that weapon - just skip this ammo
				  AmmoInfo &ai = _aaiAmmo[i];
				  ASSERT( ai.ai_iAmmoAmmount>=0);
				  ...
				  //HUD_DrawText(   fCol        , fRow+10, strValue, NONE, fNormValue);
					...
					DIO_DrawIcon(ESP_Middle,  500, ESP_Middle, iListValuePosY, *_aaiAmmo[i].ai_ptoAmmo, C_WHITE|255);
				  }
				  // advance to next position
				  fCol -= fAdvUnitS;  
				}
			...
			/*
			CTextureObject* ammoIcon = _awiWeapons[penPlayer->GetPlayerWeapons()->m_iCurrentWeapon].wi_paiAmmo->ai_ptoAmmo;
			DIO_DrawIcon(ESP_Middle,  500, ESP_Middle, iListValuePosY, ESP_Middle, 32, ESP_Middle, 32, 
				ammoIcon, 0xFFFFFFFF);
*/
		...
		/* _pDP->PutTextR( strName+":", _pixDPWidth-14*fCharWidth, fCharHeight*i+fOneUnit*4, colScore |_ulAlphaHUD);
          ...
          _pDP->PutTextC( strArmor,    _pixDPWidth-8*fCharWidth,  fCharHeight*i+fOneUnit*4, colArmor |_ulAlphaHUD);
          _pDP->PutTextC( strMoney,    _pixDPWidth-4*fCharWidth,  fCharHeight*i+fOneUnit*4, _colHUD  |_ulAlphaHUD);*/

        } else if( bScoreMatch) { 
          _pDP->PutTextR( strName+":", _pixDPWidth-14*fCharWidth, fCharHeight*i+fOneUnit*4, _colHUD  |_ulAlphaHUD);
          _pDP->PutText(  "/",         _pixDPWidth- 7*fCharWidth, fCharHeight*i+fOneUnit*4, _colHUD  |_ulAlphaHUD);
          _pDP->PutTextC( strScore,    _pixDPWidth-10*fCharWidth, fCharHeight*i+fOneUnit*4, colScore |_ulAlphaHUD);
          _pDP->PutTextC( strMana,     _pixDPWidth- 4*fCharWidth, fCharHeight*i+fOneUnit*4, colMana  |_ulAlphaHUD);
        } else { // fragmatch!
          _pDP->PutTextR( strName+":", _pixDPWidth-10*fCharWidth, fCharHeight*i+fOneUnit*4, _colHUD  |_ulAlphaHUD);
          _pDP->PutText(  "/",         _pixDPWidth-6*fCharWidth,  fCharHeight*i+fOneUnit*4, _colHUD  |_ulAlphaHUD);
          _pDP->PutTextC( strFrags,    _pixDPWidth-8*fCharWidth,  fCharHeight*i+fOneUnit*4, colFrags |_ulAlphaHUD);
          _pDP->PutTextC( strDeaths,   _pixDPWidth-4*fCharWidth,  fCharHeight*i+fOneUnit*4, colDeaths|_ulAlphaHUD);
        }
      }
    ...
    if ((bScoreMatch || bFragMatch) && hud_bShowMatchInfo){
      ...
      if (GetSP()->sp_iFragLimit>0 && _penPlayer->m_bShowingTabInfo) {
      ...
      if (GetSP()->sp_iScoreLimit>0 && _penPlayer->m_bShowingTabInfo) {
