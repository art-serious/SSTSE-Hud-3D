...
191 FLOAT m_fMoneyCoinChance "Money Coin Chance" = 0.0f,
192 FLOAT m_fMoneyBagChance "Money Bag Chance" = 0.0f,
193 FLOAT m_fMoneyChestChance "Money Chest Chance" = 0.0f,
// bekap tolko sdelai. stranno. zapusti iz pod debaga... можно бэкапить только энемибейс вот в этом виде
  {
...
components:
  ...
  6 class   CLASS_MONEYITEM       "Classes\\MoneyItem.ecl",
...
//**********************************************************
  ...
  Die(EDeath eDeath)
  {
	...
	//FLOAT fAdditionalGameDamageChance = GetGameDamageMultiplier() / 10.0f;
	FLOAT fCoinDropChance = m_fMoneyCoinChance;
	FLOAT fBagDropChance = m_fMoneyBagChance;
	FLOAT fChestDropChance = m_fMoneyChestChance;

	/*if (fCoinDropChance > 0.0f) {
		fCoinDropChance += fAdditionalGameDamageChance;
	}

	if (fBagDropChance > 0.0f) {
		fBagDropChance += fAdditionalGameDamageChance;
	}

	if (fChestDropChance > 0.0f) {
		fChestDropChance += fAdditionalGameDamageChance;
	}*/

	// TODO. Chto-to sdelat s chance > 100%, A imenno - dopolnitelniu drop
	// TODO: for
	FLOAT fValueInRange = FRnd()/* + fAdditionalGameDamageChance*/; // where it is in range [0.0 - 1.0f]
	INDEX iType = -1;
	if (fValueInRange > 0.0f && fValueInRange <= fCoinDropChance) { 
		iType = MIT_COIN;
	} else if (fValueInRange > fCoinDropChance && fValueInRange <= fBagDropChance + m_fMoneyCoinChance) {
		iType = MIT_BAG;
	} else if (fValueInRange > fCoinDropChance + fBagDropChance && 
		fValueInRange <= fBagDropChance + fCoinDropChance + fChestDropChance) {
		iType = MIT_CHEST;	
	}

	//CPrintF("TOTAL %f, HIT %f, TYPE: %i\n", fBagDropChance + fCoinDropChance + fChestDropChance, 	fValueInRange, iType);

	if (iType != -1) {
		CEntityPointer penMoneyItem = CreateEntity(GetPlacement(), CLASS_MONEYITEM);
		CMoneyItem *pmi = (CMoneyItem*)&*penMoneyItem;
		pmi->m_EhitType = (MoneyItemType)iType;
		pmi->m_bDroppedByEnemy = TRUE;

		pmi->m_fCustomRespawnTime=600.0f;
		pmi->m_bDropped=TRUE;
