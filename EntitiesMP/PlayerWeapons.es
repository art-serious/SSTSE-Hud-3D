...
static BOOL  h3d_bWeaponInertia = TRUE; //SSE
...
void CPlayerWeapons_Init(void) {
  ...
  _pShell->DeclareSymbol("persistent user INDEX h3d_bWeaponInertia;", &h3d_bWeaponInertia);
...
// USE ONLY FOR HUD.cpp
INDEX GetAmmoAmount(INDEX iHUDAmmoIndex) {
	switch (iHUDAmmoIndex) {
		case 0: return m_iShells;
		case 1: return m_iBullets;
		case 2: return m_iRockets;
		case 3: return m_iGrenades;
		case 4: return m_iNapalm;
		case 5: return m_iSniperBullets;
		case 6: return m_iElectricity;
		case 7: return m_iIronBalls;
	}
                          ...
                          BOOL bRender, INDEX iEye)
  {
    ...
    { 
      ...
      BeginModelRenderingView(apr, pdp);
	    WeaponAngleOffset(plWeaponMirror.pl_OrientationAngle);
    ...
    BeginModelRenderingView(apr, pdp);
	  ...
	  WeaponAngleOffset(plWeapon.pl_OrientationAngle);
   ...
   // * SSE ***************************************************************************************
  void WeaponAngleOffset(ANGLE3D &plAngle) { 
    if(h3d_bWeaponInertia == FALSE) { return; }
    const FLOAT fMaxAngle = 18.0f;

    CPlayer* penPlayer = GetPlayer();

    ANGLE3D aWeaponSway = ANGLE3D(0,0,0);
    aWeaponSway = Lerp(penPlayer->m_aWeaponSwayOld, penPlayer->m_aWeaponSway, _pTimer->GetLerpFactor());
    aWeaponSway(1) = Clamp(aWeaponSway(1), -fMaxAngle, fMaxAngle);
    aWeaponSway(2) = Clamp(aWeaponSway(2), -fMaxAngle/2, fMaxAngle/2);
    plAngle -= aWeaponSway;
  }
  // * END SSE ************************************************************************************

  // * H3D moving offset **************************************************************************
  void H3DMovingOffset(FLOAT3D &plPos)
  {
    CPlayerAnimator &plan = (CPlayerAnimator&)*((CPlayer&)*m_penPlayer).m_penAnimator;
    FLOAT fXOffset = Lerp(plan.m_fMoveLastBanking, plan.m_fMoveBanking, _pTimer->GetLerpFactor()) * -0.005f;
