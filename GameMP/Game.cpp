    ...
    {for (INDEX i=0; i<ctViewers; i++) {
      ...
      if (pdp!=NULL && pdp->Lock()) {
        ...
        if (apenViewers[i]!=NULL) {
          bHadViewers = TRUE;
		  // spectator pointer
		  CEntityPointer penSpectatorPlayer = ((CPlayer*)&*apenViewers[i])->m_penSpectatorPlayer;
		  ...
		  if (penSpectatorPlayer == NULL) {
			penSpectatorPlayer = ((CPlayer*)&*apenViewers[i])->m_penSpectatorPlayer;
		  }

		  //CPrintF("View %i, has spectator %i\n", i, penSpectatorPlayer);

		  if (penSpectatorPlayer!=NULL) {
			  apenViewers[i]=penSpectatorPlayer;
			  if (apenViewers[i]->IsPredicted()) {
				//CPrintF("Spectator was predicted, switched to predictor\n");
				apenViewers[i]=apenViewers[i]->GetPredictor();
			  }
		  }
...
void CGame::GameMainLoop(void)
{
  ...
  if(IsScriptSoundPlaying(5)) //if music not playin,
  {
  StopScriptSound(5); //We stop him
  }
...
{
  if(!IsScriptSoundPlaying(5))//If menu music not playing in channel 5
  {
  PlayScriptSound(5/*music channel 5*/, CTFILENAME("Music\\Menu\\Menu.ogg"), 1.0f/*volume*/, 1.0f/*wtf*/, TRUE/*loop*/); //when we stop music 
  }
...
COLOR CGame::LCDGetColor(COLOR colDefault, const char *strName)
{
  if (!strcmp(strName, "thumbnail border")) {
    ...
    colDefault = SE_COL_BLUE_DARK_LT |255;
